...
typography:
  font-sans: '"Noto Sans KR", -apple-system, BlinkMacSystemFont, "Apple SD Gothic Neo", "Malgun Gothic", Roboto, "Helvetica Neue", Arial, sans-serif'
...
Type is the second decisive voice. This system substitutes the site's existing sans stack (`{typography.font-sans}`) for Uber's two proprietary faces, so weight carries the split that two families carried in the original: **700 for headlines** (20 – 52 px display sizes with tight 1.22 – 1.4 line-height, never letter-spaced) and **400 / 500 for body, button, and link**. The voice reads as engineering-grade — no italic, no decorative weight, no tracking flourish. Headlines are sentence-case; eyebrows are uppercase only when used as the section eyebrow ("WHY BECOME"); buttons are sentence-case.
...
### Syntax Highlighting — the documented accent exception
The no-second-accent rule governs **brand chrome**: CTAs, cards, bands, chrome fills. It does not govern **code content**. A grayscale-only syntax theme costs real readability on a technical blog, so code blocks carry a desaturated multi-hue theme inside the `{colors.ink}` block — violet keywords, green strings, orange numbers, blue functions, red tags, amber properties. The hues are held low-saturation on purpose: from across the page a code block still reads as a quiet black rectangle, not as a colour accent competing with the black CTA pill. Inline code stays neutral (`{colors.canvas-soft}` fill, `{colors.ink}` text) because it sits inside running body copy, where colour would fragment the line.

## Typography

### Font Family
One sans stack carries the entire system:
...
This is the site's existing face (`--font-sans` in `src/styles/global.css`) and is deliberately unchanged by this system — Uber's `UberMove` / `UberMoveText` pair is proprietary, and the brand's voltage lives in the black-and-white duet and the pill geometry, not in the typeface. Code and monospace fragments use `{typography.font-mono}` (`--font-mono`).

The original system split display from body by *family*. With one family, the split is by **weight**, which the loaded set (400 / 500 / 700) covers exactly:

1. **Weight 700** for every headline, 20 – 52 px, never letter-spaced, line-height tightening from 1.4 down to 1.22 as size grows.
2. **Weights 400 / 500** for body, button, link, and small headings, 12 – 18 px. Tracking always neutral.

The roles never overlap: 700 never carries a body paragraph; 400 / 500 never carries a hero headline.
...
| `{typography.code}` | 14px | 400 | 22px | Inline code and code blocks — `{typography.font-mono}`. |
...
- **No tracking flourish.** The display weight is never letter-spaced, positive or negative.
- **One face, two weight roles.** 700 for display; 400 / 500 for everything else. Never cross the streams.
- **Korean body copy holds its line-height.** The 24 px / 16 px ratio (1.5) on `{typography.body-md}` is the floor for running Hangul; don't tighten it toward the 1.25 used on button labels.

### Note on Font Substitutes
The original two faces are proprietary and are **not** used here; the site keeps its own stack (above). If the brand's own faces ever become available, the mapping is: `UberMove` 700 → every `display-*` token; `UberMoveText` 400 / 500 → every `body-*` / `button-*` / `caption` token. Other open-source approximations, in order: *Inter* (700 display, 400 / 500 text), *Geist* 700, *Plus Jakarta Sans* for a softer text face.
...
- **Reading column**: long-form article body is the one exception to the 1200 px container — it caps at ~700 px so the measure stays readable. Every other surface (nav, hero, promo rows, list pages, footer) uses the full 1200 px container.
- **Column patterns**:
...
- **Display sizes**: `{typography.display-xxl}` and `{typography.display-xl}` scale down fluidly on narrow viewports; line-height is carried as a ratio (64/52 ≈ 1.23, 44/36 ≈ 1.22) so the tight display leading survives the scale.
...
- Set every headline in `{typography.display-*}` weight 700 in sentence-case. Weight 700 never carries body copy.
...
- Keep the article reading column at ~700 px while every other surface uses the 1200 px container.

### Don't
- Don't introduce a second brand accent colour (orange, blue, green). The brand's entire UI is black-and-white plus grayscale; new accents flatten the system. Syntax highlighting inside code blocks is the one documented exception — see **Syntax Highlighting** above.
...
- Don't tighten or loosen letter-spacing on the display weight. The brand never letter-spaces; default tracking is part of the voice.
...
- Don't introduce a second typeface. `{typography.font-sans}` carries every text role; `{typography.font-mono}` is only for code.
